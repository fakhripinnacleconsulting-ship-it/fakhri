...
import {
    ...
    Search, Filter, MoreVertical, File as FileIcon, Users,
...
const AdminFilesTab = ({ currentUser }) => {
    ...
    const [recipientSortOrder, setRecipientSortOrder] = useState("asc");
    ...
    async function loadData() {
        ...
        try {
            ...
            const clientsFilter = isSuperAdmin ? {} : { managerId: currentUser._id };
            const clientsRes = await getClients(clientsFilter);
            const clientsData = clientsRes.clients || [];
            const myClientIds = clientsData.map(c => c._id);

            // Get files for my clients OR shared directly with me OR uploaded by me
            const filesFilter = isSuperAdmin
                ? {}
                : {
                    $or: [
                        { clientId: { $in: myClientIds } },
                        { recipientId: { $in: [...myClientIds, currentUser._id] } },
                        { uploadedBy: currentUser.name }
                    ]
                };

            const filesData = await getFiles(filesFilter);
            setFiles(filesData);
    ...
    const filteredFiles = useMemo(() => {
        let result = files.filter(file => {
            ...
            if (typeFilter !== "all") {
                ...
                if (typeFilter === "image" && !['jpg', 'jpeg', 'png', 'svg', 'webp', 'image'].includes(ft)) return false;
                if (typeFilter === "pdf" && ft !== "pdf") return false;
                if (typeFilter === "excel" && !['xls', 'xlsx', 'csv', 'excel'].includes(ft)) return false;
                if (typeFilter === "other" && ['jpg', 'jpeg', 'png', 'svg', 'webp', 'image', 'pdf', 'xls', 'xlsx', 'csv', 'excel'].includes(ft)) return false;
            }
        ...
        return result;
    }, [dateRange, files, searchQuery, sortConfig, typeFilter, sourceFilter]);
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchQuery, dateRange, typeFilter, sourceFilter]);
    ...
    const handleUploadSubmit = async () => {
        ...
        try {
            ...
            // Build recipients array with role field (required by uploadFiles action)
            const recipients = clients
                .filter(c => selectedClientIds.includes(c._id))
                .map(c => ({ id: c._id, name: c.name, role: 'client' }));
            ...
            if (result.success) {
                toast.success(`Successfully uploaded and shared with ${recipients.length} client(s)`);
    ...
    return (
        <div className="h-[calc(100vh-85px)] flex flex-col space-y-2 overflow-hidden">
            {/* Filter Bar: Unified Search, Date Filters and Upload */}
            <div className="flex flex-col lg:flex-row lg:items-center gap-3 bg-card rounded-xl border shadow-sm p-2 shrink-0">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <div className="flex items-center gap-2 bg-muted/30 px-3 py-1.5 rounded-lg border border-dashed">
                    ...
                    <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
                        <DialogTrigger asChild>
                            <Button className="bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 h-10 px-6 font-bold rounded-lg transition-all active:scale-95">
                        ...
                        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-hidden flex flex-col">
                            <DialogHeader>
                                <DialogTitle>Upload & Share File</DialogTitle>
                                <DialogDescription>
                                    Upload a document and select which clients should receive it.
                                </DialogDescription>
                            </DialogHeader>
                            ...
                            <DialogFooter className="bg-muted/30 -mx-6 -mb-6 p-4 mt-auto border-t">
                                ...
                                >
                                    {uploading ? (
                                        ...
                                        <><CheckCircle2 className="mr-2 h-4 w-4" />Send to Clients</>
                                    )}
                                </Button>
                            </DialogFooter>
                        </DialogContent>
            ...
            <div className="flex-1 min-h-0 bg-card rounded-xl border shadow-sm overflow-hidden flex flex-col">
                    ...
                    <Table wrapperClassName="overflow-visible">
                        <TableHeader className="sticky top-0 z-30 bg-muted/90 backdrop-blur-md border-b shadow-sm">
                            <TableRow className="hover:bg-transparent border-none">
                                    ...
                                    <div className="flex flex-col gap-1.5 py-2">
                                        ...
                                        <Select value={typeFilter} onValueChange={setTypeFilter}>
                                            ...
                                            <SelectContent>
                                                ...
                                                <SelectItem value="pdf" className="text-[10px]">PDF</SelectItem>
                                                <SelectItem value="excel" className="text-[10px]">Excel</SelectItem>
                                ...
                                <TableHead className="text-right bg-muted/50 font-bold uppercase text-[11px] tracking-wider sticky top-0 z-30">Actions</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                                    ...
                                    <TableRow key={file._id} className="hover:bg-accent/5 transition-colors group">
                                        <TableCell>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col min-w-0">
                                                    <span className="font-semibold text-sm truncate max-w-[200px]" title={file.name}>
                                                        {file.name}
                                                    </span>
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-bold capitalize truncate max-w-[140px]">
                                                        {(file.recipientId === currentUser._id || file.clientId === currentUser._id) ? "You (Admin)" : (file.recipientName || file.clientName)}
                                                    </span>
                                                </div>
                                                <div className="flex gap-1">
                                                    <Badge
                                                        ...
                                                        className={cn(
                                                            ...
                                                            (file.recipientId === currentUser._id) ? "bg-purple-100 text-purple-700 hover:bg-purple-100" : "bg-blue-100 text-blue-700 hover:bg-blue-100"
                                                        )}
                                                    >
                                                        {(file.recipientId === currentUser._id) ? 'direct' : (file.recipientRole || 'client')}
                                                    </Badge>
                                                </div>
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="text-xs font-semibold">{file.date || new Date(file.createdAt).toLocaleDateString()}</span>
                                    ...
                                    <TableCell colSpan={7} className="text-center py-20 text-muted-foreground bg-accent/5">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <div className="space-y-1">
                                                <p className="text-lg font-bold">No files found</p>
            ...
            <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            File shared with {selectedFile?.recipientName || selectedFile?.clientName} on {selectedFile?.date}
                        </DialogDescription>
                    </DialogHeader>
