...
export async function upsertTask(taskData) {
    ...
    try {
        ...
        // 1. Plan Expiry Check (for New Tasks)
        if (isNew && updateData.client?.id) {
            const client = await User.findById(updateData.client.id).select('subscriptionEnd subscribedServices role').lean();
            if (client && client.role === 'client') {
                ...
                if (!isPlanActive && !hasActiveAddons) {
                    return {
                        success: false,
                        error: "This client's plan has expired and they have no active add-on services. Task creation is disabled."
                    };
                }
            }
        }
