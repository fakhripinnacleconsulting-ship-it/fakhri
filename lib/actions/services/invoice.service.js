...
export async function getInvoices({
    ...
    const query = { isManualAmount: { $ne: true }, isDeleted: { $ne: true } };
...
export async function createInvoice(invoiceData) {
    ...
    try {
        ...
        // Incorporate excludeFromRevenue if provided
        if (invoiceData.excludeFromRevenue !== undefined) {
            invoiceData.excludeFromRevenue = invoiceData.excludeFromRevenue === true;
        }
...
export async function deleteInvoice(id, removeFromRevenue = true) {
    await validateRole(['super-admin', 'admin']);
    await connectDB();
    try {
        if (removeFromRevenue) {
