    ...
    return await cache.wrap(cacheKey, async () => {
        ...
        try {
            ...
            const getRevenuePipeline = (periodStart, periodEnd) => {
                const matchQuery = { status: 'Paid', isManualAmount: { $ne: true } };
            ...
            const getManualRevenuePipeline = (periodStart, periodEnd) => {
                const matchQuery = { role: 'client', amount: { $gt: 0 } };
                if (periodStart || periodEnd) {
                    const dateQuery = {};
                    if (periodStart) dateQuery.$gte = periodStart;
                    if (periodEnd) dateQuery.$lte = periodEnd;
            ...
            const [currRevenueResult, currManualResult, pendingInvoicesCount] = await Promise.all([
                Invoice.aggregate(getRevenuePipeline(start, end)),
                User.aggregate(getManualRevenuePipeline(start, end)),
                Invoice.countDocuments({ status: 'Pending' })
            ]);

            totalRevenue = (currRevenueResult.length > 0 ? currRevenueResult[0].totalAmount : 0) + 
                           (currManualResult.length > 0 ? currManualResult[0].totalAmount : 0);

            if (start && end) {
                const { prevStart, prevEnd } = getPreviousPeriodRange(start, end);
                const [prevRevenueResult, prevManualResult] = await Promise.all([
                    Invoice.aggregate(getRevenuePipeline(prevStart, prevEnd)),
                    User.aggregate(getManualRevenuePipeline(prevStart, prevEnd))
                ]);
                prevRevenue = (prevRevenueResult.length > 0 ? prevRevenueResult[0].totalAmount : 0) +
                              (prevManualResult.length > 0 ? prevManualResult[0].totalAmount : 0);
            }
