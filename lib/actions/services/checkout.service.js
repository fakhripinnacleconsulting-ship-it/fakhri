...
export async function processCheckout({ userId, planId, cartItems, billingDetails, totalAmount, paymentId, orderId, signature, couponCode, upgradeDiscount = 0, currentPlan = '' }) {
    await connectDB();
    ...
    try {
        ...
        if (planId) {
            ...
            invoiceItems.push({
                ...
                hsnCode: planDetails?.hsnCode || '998311',
                qty: 1,
                price: parsedPrice,
                total: parsedPrice
            });
        ...
        // 2.6 Process Upgrade Discount
        if (upgradeDiscount > 0) {
            invoiceItems.push({
                description: `Plan Upgrade Discount (75% remaining value${currentPlan ? ` from ${currentPlan}` : ''})`,
                qty: 1,
                price: -upgradeDiscount,
                total: -upgradeDiscount
            });
        }
        ...
        if (newPlan) {
            ...
            if (planDoc) {
                ...
                const now = new Date();
                const durationValue = Number(planDoc.durationValue) || 1;
                const durationUnit = planDoc.durationUnit || 'month';

                let startDate = now;
                // If user has an existing active subscription, extend it
                ...
                updateData.subscriptionStart = user.subscriptionStart || now;
