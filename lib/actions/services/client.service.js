...
export async function upsertClient(clientData) {
    ...
    try {
        ...
        if (id && id.toString().length >= 12) {
            ...
            if (updateData.subscriptionStart === "") updateData.subscriptionStart = null;
            if (updateData.subscriptionEnd === "") updateData.subscriptionEnd = null;
            if (updateData.joinedDate === "") updateData.joinedDate = null;

            // Ensure subscription dates are set if there's an amount/plan
            if ((updateData.amount && updateData.amount > 0) || (updateData.plan && updateData.plan !== 'None')) {
                if (!updateData.subscriptionStart) updateData.subscriptionStart = new Date();
                if (!updateData.subscriptionEnd) {
                    const end = new Date(updateData.subscriptionStart);
            ...
            if (client) {
                // Clear cache on any amount change to ensure revenue updates
                if (updateData.amount !== undefined) {
                    await cache.clear();
                }
            }
            ...
            if ((updateData.plan && updateData.plan !== 'None') || (updateData.amount && Number(updateData.amount) > 0)) {
                if (!updateData.subscriptionStart) {
                    updateData.subscriptionStart = new Date();
                }
                if (!updateData.subscriptionEnd) {
                    const end = new Date(updateData.subscriptionStart);
                    end.setDate(end.getDate() + 30); // Default to 30 day billing cycle
                    updateData.subscriptionEnd = end;
                }
            }
            ...
            if (client) {
                // Clear cache so any new manual amounts factor into revenue immediately
...
export async function bulkUploadClients(clientsData) {
    ...
    try {
        ...
        // Clear cache once after bulk upload if any client had an amount
        const wasRevenueInfluenced = clientsData.some(c => Number(c.amount) > 0);
        if (wasRevenueInfluenced) {
            try { await cache.clear(); } catch (e) { console.error("Cache clear failure:", e); }
        }
